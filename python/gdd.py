...
cfg = {'email':'','password':'','path':'','override':False,'format':'mso','feed':'https://docs.google.com/feeds/documents/private/full','document_format':'','spreadsheet_format':'','presentation_format':'','drawing_format':'','use_unicode':False,'use_datetag':False,'use_folders':False,'modified_only':False,'clean_up':False,'illegal_characters':'<>:"/\|?*^'}
...
def login():
	...
	values = {'Email' : cfg['email'], 'Passwd' : cfg['password'], 'accountType' : 'HOSTED_OR_GOOGLE', 'service' : 'writely', 'source' : user_agent, 'GData-Version': '2.0' }
	values2 = {'Email' : cfg['email'], 'Passwd' : cfg['password'], 'accountType' : 'HOSTED_OR_GOOGLE', 'service' : 'wise', 'source' : user_agent, 'GData-Version': '2.0' }
...
def getFeed(auth,feed,tries=0):
	...
	try:
		...
		req.add_header('GData-Version','2.0')
		response = urllib2.urlopen(req)
...
def download(auth,urls):
	...
	for i in docs:
		if i['type'] != "spreadsheet":
			result = getFile(i,{'Authorization':'GoogleLogin auth=' + auth[1], 'GData-Version': '2.0'}, cfg['path']+auth[0]+dtpath)
			noerror = result['success'] and noerror
			files.append(result['file'])
		else:
			result = getFile(i,{'Authorization':'GoogleLogin auth=' + auth[2], 'GData-Version': '2.0'}, cfg['path']+auth[0]+dtpath)
			noerror = result['success'] and noerror
			files.append(result['file'])
